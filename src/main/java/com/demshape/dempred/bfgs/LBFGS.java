 ...
 */
package com.demshape.dempred.bfgs;

/**
 * <p>
 * This class contains code for the limited-memory Broyden-Fletcher-Goldfarb-Shanno (LBFGS) algorithm for large-scale multidimensional unconstrained minimization problems. This file is a translation of Fortran code written by Jorge Nocedal. The only modification to the algorithm is the addition of a cache to store the result of the most recent line search. See <tt>solution_cache</tt> below.
 * 
 ...
 * 
 * <p>
 * This code is derived from the Fortran program <code>lbfgs.f</code>. The Java translation was effected mostly mechanically, with some manual clean-up; in particular, array indices start at 0 instead of 1. Most of the comments from the Fortran code have been pasted in here as well.
 * </p>
 * 
 * <p>
 * Here's some information on the original LBFGS Fortran source code, available at <a href="http://www.netlib.org/opt/lbfgs_um.shar"> http://www.netlib.org/opt/lbfgs_um.shar</a>. This info is taken verbatim from the Netlib blurb on the Fortran source.
 * </p>
 * 
 * <pre>
 ...
 * </pre>
 * 
 * @author Jorge Nocedal: original Fortran version, including comments (July 1990). Robert Dodier: Java translation, August 1997.
 */

public class LBFGS {
	/**
	 * Specialized exception class for LBFGS; contains the <code>iflag</code> value returned by <code>lbfgs</code>.
	 */

	public Mcsrch mcsrch;

	public LBFGS() {
		mcsrch = new Mcsrch();
		mcsrch.setLbfgs(this);
	}
	 ...
	 * </pre>
	 * 
	 * using the limited-memory BFGS method. The routine is especially effective on problems involving a large number of variables. In a typical iteration of this method an approximation <code>Hk</code> to the inverse of the Hessian is obtained by applying <code>m</code> BFGS updates to a diagonal matrix <code>Hk0</code>, using information from the previous M steps. The user specifies the number <code>m</code>, which determines the amount of storage required by the routine. The user may also
	 * provide the diagonal matrices <code>Hk0</code> if not satisfied with the default choice. The algorithm is described in "On the limited memory BFGS method for large scale optimization", by D. Liu and J. Nocedal, Mathematical Programming B 45 (1989) 503-528.
	 * 
	 * The user is required to calculate the function value <code>f</code> and its gradient <code>g</code>. In order to allow the user complete control over these computations, reverse communication is used. The routine must be called repeatedly under the control of the parameter <code>iflag</code>.
	 * 
	 * The steplength is determined at each iteration by means of the line search routine <code>mcsrch</code>, which is a slight modification of the routine <code>CSRCH</code> written by More' and Thuente.
	 * 
	 * The only variables that are machine-dependent are <code>xtol</code>, <code>stpmin</code> and <code>stpmax</code>.
	 * 
	 * Progress messages are printed to <code>System.out</code>, and non-fatal error messages are printed to <code>System.err</code>. Fatal errors cause exception to be thrown, as listed below.
	 * 
	 * @param n
	 *            The number of variables in the minimization problem. Restriction: <code>n &gt; 0</code>.
	 * 
	 * @param m
	 *            The number of corrections used in the BFGS update. Values of <code>m</code> less than 3 are not recommended; large values of <code>m</code> will result in excessive computing time. <code>3 &lt;= m &lt;= 7</code> is recommended. Restriction: <code>m &gt; 0</code>.
	 * 
	 * @param x
	 *            On initial entry this must be set by the user to the values of the initial estimate of the solution vector. On exit with <code>iflag = 0</code>, it contains the values of the variables at the best point found (usually a solution).
	 * 
	 * @param f
	 *            Before initial entry and on a re-entry with <code>iflag = 1</code>, it must be set by the user to contain the value of the function <code>f</code> at the point <code>x</code>.
	 * 
	 * @param g
	 *            Before initial entry and on a re-entry with <code>iflag = 1</code>, it must be set by the user to contain the components of the gradient <code>g</code> at the point <code>x</code>.
	 * 
	 * @param diagco
	 *            Set this to <code>true</code> if the user wishes to provide the diagonal matrix <code>Hk0</code> at each iteration. Otherwise it should be set to <code>false</code> in which case <code>lbfgs</code> will use a default value described below. If <code>diagco</code> is set to <code>true</code> the routine will return at each iteration of the algorithm with <code>iflag = 2</code>, and the diagonal matrix <code>Hk0</code> must be provided in the array <code>diag</code>.
	 * 
	 * @param diag
	 *            If <code>diagco = true</code>, then on initial entry or on re-entry with <code>iflag = 2</code>, <code>diag</code> must be set by the user to contain the values of the diagonal matrix <code>Hk0</code>. Restriction: all elements of <code>diag</code> must be positive.
	 * 
	 * @param iprint
	 *            Specifies output generated by <code>lbfgs</code>. <code>iprint[0]</code> specifies the frequency of the output:
	 *            <ul>
	 ...
	 *            <ul>
	 *            <li> <code>iprint[1] = 0</code>: iteration count, number of function evaluations, function value, norm of the gradient, and steplength,
	 *            <li> <code>iprint[1] = 1</code>: same as <code>iprint[1]=0</code>, plus vector of variables and gradient vector at the initial point,
	 *            <li> <code>iprint[1] = 2</code>: same as <code>iprint[1]=1</code>, plus vector of variables,
	 *            <li> <code>iprint[1] = 3</code>: same as <code>iprint[1]=2</code>, plus gradient vector.
	 *            </ul>
	 * 
	 * @param eps
	 *            Determines the accuracy with which the solution is to be found. The subroutine terminates when
	 * 
	 *            <pre>
	 *            ||G|| &lt; EPS max(1,||X||),
	 * </pre>
	 ...
	 * 
	 * @param xtol
	 *            An estimate of the machine precision (e.g. 10e-16 on a SUN station 3/60). The line search routine will terminate if the relative width of the interval of uncertainty is less than <code>xtol</code>.
	 * 
	 * @param iflag
	 *            This must be set to 0 on initial entry to <code>lbfgs</code>. A return with <code>iflag &lt; 0</code> indicates an error, and <code>iflag = 0</code> indicates that the routine has terminated without detecting errors. On a return with <code>iflag = 1</code>, the user must evaluate the function <code>f</code> and gradient <code>g</code>. On a return with <code>iflag = 2</code>, the user must provide the diagonal matrix <code>Hk0</code>.
	 * 
	 *            The following negative values of <code>iflag</code>, detecting an error, are possible:
	 *            <ul>
	 *            <li> <code>iflag = -1</code> The line search routine <code>mcsrch</code> failed. One of the following messages is printed:
	 *            <ul>
	 *            <li>Improper input parameters.
	 *            <li>Relative width of the interval of uncertainty is at most <code>xtol</code>.
	 ...
	 *            <li>Rounding errors prevent further progress. There may not be a step which satisfies the sufficient decrease and curvature conditions. Tolerances may be too small.
	 *            </ul>
	 *            <li><code>iflag = -2</code> The i-th diagonal element of the diagonal inverse Hessian approximation, given in DIAG, is not positive.
	 *            <li><code>iflag = -3</code> Improper input parameters for LBFGS (<code>n</code> or <code>m</code> are not positive).
	 *            </ul>
	 * 
	...
	/**
	 * Print debugging and status messages for <code>lbfgs</code>. Depending on the parameter <code>iprint</code>, this can include number of function evaluations, current function value, etc. The messages are output to <code>System.out</code>.
	 * 
	 * @param iprint
	...
	/**
	 * Compute the sum of a vector times a scalara plus another vector. Adapted from the subroutine <code>daxpy</code> in <code>lbfgs.f</code>. There could well be faster ways to carry out this operation; this code is a straight translation from the Fortran.
	 */
	...
	/**
	 * Compute the dot product of two vectors. Adapted from the subroutine <code>ddot</code> in <code>lbfgs.f</code>. There could well be faster ways to carry out this operation; this code is a straight translation from the Fortran.
	 */
